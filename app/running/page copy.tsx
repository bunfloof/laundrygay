...
const Running: React.FC = () => {
  ...
  return (
    <div className="flex flex-col items-center justify-center bg-background min-h-screen px-4" style={{ minHeight: 'calc(var(--vh, 1vh) * 100)' }}>
      <div className="w-full max-w-md flex flex-col items-start">
        <div className="mb-2 self-start">
          <Button
            variant="link2"
            onClick={() => router.back()}
            className="flex items-center text-foreground/50 hover:text-foreground transition-colors duration-200"
          >
        ...
        <Card className="w-full border shadow-none">
          <CardContent className="p-6">
            <div className="flex justify-between items-center mb-4">
              <h2 className="text-2xl font-bold">Running Machines</h2>
              {runningMachines.length > 0 && (
                <span 
                  className="text-md text-red-600 hover:text-red-700 cursor-pointer transition-colors"
                  onClick={() => {
                    setMachineToDelete(null);
                    setIsAlertOpen(true);
                  }}
            ...
            {runningMachines.length === 0 ? (
              <p className="text-center text-gray-500">View all of the laundry machines you’ve started.</p>
            ) : (
